...
class LLRB {
	...
	Node* put(Node* h, Key key, Value val) {
		if (h == nullptr) return new Node(key, val, 1, RED);                    // if h is null, return a new node with default count of 1
    ...
    if (key < x->key) 
        ...
        x->right = removeMin(t->right);                                         // Delete the minimum node from the right subtree
